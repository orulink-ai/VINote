...
Stopping saves its blob and PendingMeeting metadata in IndexedDB and releases capture resources. Recording-only mode returns to /meetings without STT/LLM; minutes mode automatically generates and saves the note, using complete media when live transcription is unavailable or the recording is video. LocalRecordingCard owns replay/download/confirmed deletion/later generation. A saved recording is visible without a note; generation removes its local pending copy only after the server note has been saved. Preserve actual recording start/end separately from generation time and meeting wall-clock context.
...
- `minutes` mode automatically generates after durable local saving. Audio with healthy finalized live speech submits its transcript together with the original recording; `/api/generate_from_upload` accepts an optional authenticated `recording` multipart attachment for transcript input. Missing/failed realtime speech falls back to full media transcription. Video always submits complete media. `recording` mode only saves locally.
- The deployed live cloud `session.start` request forbids `profileId` but requires explicit `model`; the backend resolves and validates the current ASR choice and returns it from `/api/vilab/realtime-connection`. Final transcript segments must be read after realtime completion. Failed startup rejects the client promise.
