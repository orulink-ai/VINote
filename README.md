...
VINote 支持独立 Langfuse 项目的笔记生成链路追踪，覆盖媒体准备、转写、分块总结、LLM 调用、截图与结果保存。配置、安装版接入和真实上报验证见 [Langfuse 接入说明](docs/langfuse.md)。源码、测试包与正式包固定启用，默认不采集正文。两种包自动携带构建者提供的 Langfuse 项目配置，并强制验证上报与读回；无需安装后手动配置。凭据分发边界见接入说明。
