...
export function Meetings() {
  ...
  return <div className="mx-auto flex max-w-[1320px] flex-col gap-6 px-6 py-8 pb-32 lg:px-10">
    <header className="flex flex-wrap items-end justify-between gap-4 border-b pb-5">
      <div><div className="mb-2 text-sm text-muted-foreground">{workspace}</div><h1 className="text-2xl font-semibold tracking-tight">{zh ? '会议记录' : 'Meeting capture'}</h1>
        <p className="mt-1 text-sm text-muted-foreground">{zh ? '先可靠保存录制，再按需生成逐字稿、纪要、决策和待办。' : 'Save the recording first, then generate transcript, minutes, decisions and actions.'}</p></div>
    ...
    <section className="grid gap-6 lg:grid-cols-[340px_minmax(0,1fr)]">
      ...
      <header><h2 className="flex items-center gap-2 font-semibold"><SlidersHorizontal className="size-4" />{zh ? '录制设置' : 'Recording setup'}</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">{recordingMode === 'smart' ? (zh ? '停止后先保存本地录制，再由你确认生成逐字稿和纪要。' : 'The recording is saved first; you choose when to generate transcript and minutes.') : (zh ? '只保存本地录制，不调用转写和总结。' : 'Save locally without transcription or summarization.')}</p></header>
      ...
      <section className="flex min-h-[540px] flex-col overflow-hidden border"><header className="border-b p-5"><div className="flex items-center justify-between"><div><h2 className="font-semibold">{zh ? '对话记录' : 'Conversation'}</h2><p className="mt-1 text-sm text-muted-foreground">{zh ? '录制时显示状态，转写完成后按说话人展示。' : 'Shows capture status, then speaker messages after transcription.'}</p></div>{busy ? <Badge variant="destructive">{copy.meetingRecorder.phases[session.phase]}</Badge> : <Badge variant="secondary">{zh ? '准备就绪' : 'Ready'}</Badge>}</div></header>
    ...
    <section className="grid gap-3 border-t pt-6"><h2 className="font-semibold">{zh ? '已保存的录制' : 'Saved recordings'}</h2>
