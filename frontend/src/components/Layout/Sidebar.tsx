...
import { ChevronUp, FileInput, Library, Mic2, Settings2, Sparkles, Users } from 'lucide-react'
...
export function Sidebar() {
  ...
  return <ShadcnSidebar collapsible="icon" variant="sidebar" className="border-r-0">
    <SidebarHeader className="border-b px-3 py-3"><SidebarMenu><SidebarMenuItem><SidebarMenuButton size="lg" onClick={() => navigate('/')} tooltip="VINote" className="transition-colors duration-200"><span className="flex aspect-square size-8 items-center justify-center rounded-md bg-foreground text-[11px] font-bold tracking-tight text-background">VI</span><span className="grid flex-1 text-left leading-tight"><span className="truncate text-sm font-semibold">VINote</span><span className="truncate text-xs text-muted-foreground">{zh ? '会议与知识空间' : 'Meeting knowledge space'}</span></span></SidebarMenuButton></SidebarMenuItem></SidebarMenu></SidebarHeader>
    <SidebarContent>
