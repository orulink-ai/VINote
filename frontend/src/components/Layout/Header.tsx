import { FileUp, Link, LogOut, Mic2, Moon, Plus, Search, Settings2, Sun } from 'lucide-react'
...
export function Header() {
  ...
  return <>
    <header className="flex h-12 shrink-0 items-center gap-2 border-b bg-background px-3">
      ...
      <div className="min-w-0"><p className="truncate text-sm font-medium">{zh ? current?.[1] : current?.[2]}</p></div>
      <Button variant="ghost" className="mx-auto hidden h-8 w-full max-w-md justify-start rounded-md border border-transparent bg-muted/60 text-muted-foreground hover:border-border hover:bg-muted lg:flex" onClick={() => setSearchOpen(true)}><Search data-icon="inline-start" /><span className="flex-1 text-left">{copy.header.searchPlaceholder}</span><kbd className="rounded border bg-background px-1.5 py-0.5 text-[10px]">Ctrl K</kbd></Button>
      <div className="ml-auto flex items-center gap-1">
        <DropdownMenu><DropdownMenuTrigger asChild><Button size="sm"><Plus data-icon="inline-start" />{copy.header.newButton}</Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-72"><DropdownMenuLabel>{zh ? '新建' : 'Create'}</DropdownMenuLabel>{actions.map(item => <DropdownMenuItem key={item.path} onClick={() => navigate(item.path)} className="items-start gap-3 py-3"><item.icon className="mt-0.5" /><span><span className="block font-medium">{item.label}</span><span className="block text-xs text-muted-foreground">{item.hint}</span></span></DropdownMenuItem>)}</DropdownMenuContent></DropdownMenu>
