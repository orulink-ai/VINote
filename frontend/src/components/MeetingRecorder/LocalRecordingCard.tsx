...
export function LocalRecordingCard({ recording, busy, onDeleted }: { recording: PendingMeeting; busy: boolean; onDeleted: () => void }) {
  ...
  return <article className="grid gap-4 border-b py-5 first:pt-0 last:border-b-0"><header className="flex items-start justify-between gap-4"><div className="flex min-w-0 gap-3"><div className="grid size-9 shrink-0 place-items-center border bg-muted/40"><Icon className="size-4" /></div><div className="min-w-0"><h3 className="truncate text-sm font-medium">{title}</h3><p className="mt-1 text-xs text-muted-foreground">{new Date(recording.startedAt).toLocaleString()} · {Math.floor(recording.elapsedSeconds / 60)}:{String(recording.elapsedSeconds % 60).padStart(2, '0')}</p></div></div><Badge variant="secondary">{recording.options.screen ? (zh ? '录屏' : 'Video') : (zh ? '录音' : 'Audio')}</Badge></header>
    <div className="grid gap-4">{url ? recording.options.screen ? <video src={url} controls className="max-h-80 w-full border bg-muted" /> : <audio src={url} controls className="w-full" /> : null}
      ...
      </div>{error ? <Alert variant="destructive"><AlertDescription>{error}</AlertDescription></Alert> : null}</div></article>
}
