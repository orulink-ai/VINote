...
export function ModelSourcePanel({ compact = false }: { compact?: boolean }) {
  ...
  return <div className="grid gap-6">
    <Card><CardHeader><div className="flex items-start justify-between gap-4"><div><CardTitle>{compact ? (zh ? '转写与总结模型' : 'Transcription and summary models') : (zh ? '模型运行方式' : 'Model runtime')}</CardTitle><CardDescription>{zh ? '云端模式直接使用部署模型，本地模式使用你自己的模型服务。' : 'Use deployed cloud models or connect your own local services.'}</CardDescription></div>{config?.mode === 'cloud' && <Button variant="ghost" size="sm" disabled={loading || saving} onClick={() => setRefresh(value => value + 1)}><RefreshCw className={loading ? 'animate-spin' : ''} />{zh ? '刷新' : 'Refresh'}</Button>}</div></CardHeader><CardContent className="grid gap-5">
